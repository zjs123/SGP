...
class HeavyPrompt(LightPrompt):
    ...
    def Tune(self, train_loader, gnn, answering, lossfn, opi, device):
        ...
        for batch_id, train_batch in enumerate(train_loader): 
            ...
            if train_batch.y != None:
                train_loss = lossfn(pre, train_batch.y)
            elif train_batch.edge_y != None:
                train_loss = lossfn(pre, train_batch.edge_y)
            train_loss.backward()
            opi.step()
    ...
    def Eva_Node(self, loader, data, idx_test,  gnn, answering, num_class, device, center = None):
        ...
        with torch.no_grad(): 
            for batch_id, batch in enumerate(loader): 
                ...
                prompted_graph = self.forward(batch)
                out = gnn(prompted_graph.x, prompted_graph.edge_index, prompted_graph.batch)
                out = answering(out)
        ...
        roc = auroc.compute()
        prc = auprc.compute() 
        return {'task_prompt': 'All in one', 'ACC':acc.item(), 'Macro_F1':macro.item(), 'Micro_F1':micro.item(), 'Weighted_F1':weighted.item(), 'ROC':roc.item(), 'PRC':prc.item()}
    ...
    def Eva_Graph(self, loader, data, idx_test,  gnn, answering, num_class, device, center = None):
        ...
        roc = auroc.compute()
        prc = auprc.compute()
        return {'task_prompt': 'All_in_one', 'ACC':acc.item(), 'Macro_F1':macro.item(), 'Micro_F1':micro.item(), 'Weighted_F1':weighted.item(), 'ROC':roc.item(), 'PRC':prc.item()}
