...
from torch_geometric.nn import global_mean_pool, GATConv, GCNConv, GINConv, SAGEConv, TransformerConv
    ...
    def __init__(self, input_dim, hid_dim=None, out_dim=None, num_layer=3, JK="last", drop_ratio=0, pool='mean'):
        ...
        if pool == "sum":
            ...
            self.pool = global_max_pool
             ...
             self.pool = GlobalAttention(gate_nn=torch.nn.Linear(out_dim, 1))
        else:
            raise ValueError("Invalid graph pooling type.")

      

    def forward(self, x, edge_index, batch = None, prompt = None, task_train_type = None):
        h_list = [x]
...
class GCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch = None, prompt = None, task_train_type = None, edge_weight = None):
        h_list = [x]
        for idx, conv in enumerate(self.conv_layers[0:-1]):
