...
['PubMed', 'sector_cat', 3, 3, '/home/jz875/project/label_prompt/pre_trained_model/PubMed/text.GraphMAE.GCN.128.2.pth']
['task_prompt', ['label', 'label', 'label', 'label', 'label']]
['ACC', 0.6974, 0.0255]
...
['PubMed', 'sector_cat', 1, 3, '/home/jz875/project/label_prompt/pre_trained_model/PubMed/text.GraphMAE.GCN.128.2.pth']
['task_prompt', ['label', 'label', 'label', 'label', 'label']]
['ACC', 0.5881, 0.0921]
...
from torch_geometric.utils import add_random_edge, dropout_edge, batched_negative_sampling, softmax, to_dense_batch, dense_to_sparse
...
class label_prompt_graph(torch.nn.Module):
    def __init__(self, feature_dim, in_channels, n_classes, dataset, device):
        super(label_prompt_graph, self).__init__()
    ...
    def get_critical_emb(self, gnn, x, edge_index, batch):
        
        new_x = x + self.feature_prompt
    ...
    def MI_loss(self, gnn, data, train_idx = None):
        # max_MI (z|y)
        ...
        train_labels = data.y # batch_size | 1
        ...
        for i in range(self.perturb_times):
            ...
            critical_embedding_boundary = self.get_critical_emb(gnn, boundary_x.detach(), boundary_edge_index, boundary_batch)
        ...
        boundary_features_critical = torch.stack(critical_embedding_boundary_list, dim = 0) # perturb_times | (shot_num*n_classes) | emb_dim

        # keep boundary samples
        boundary_features = boundary_features.view(train_labels.size()[0], self.perturb_times, -1, self.in_channels) # batch_size(n_classes) | perturb_times | shot_num | emb_dim
        ...
        if self.dataset in ['BZR', 'PROTEINS']:
        ...
        if self.dataset in ['COX2', 'MUTAG']:
            noise_loss = F.relu(-neg_distance+10).mean() + F.relu(neg_distance-pos_distance).mean()
        
        # min_MI
        boundary_features_critical = boundary_features_critical.view(train_labels.size()[0], self.perturb_times, -1, self.in_channels) # batch_size(n_classes) | perturb_times | shot_num | emb_dim
        ...
        if self.dataset in ['BZR', 'PROTEINS', 'MUTAG']:
    ...
    def Eva_Graph(self, loader, data, idx_test,  gnn, answering, num_class, device, center = None):
        ...
        roc = auroc.compute()
        prc = auprc.compute()
        return {'task_prompt': 'label_prompt', 'ACC':acc.item(), 'Macro_F1':macro.item(), 'Micro_F1':micro.item(), 'Weighted_F1':weighted.item(), 'ROC':roc.item(), 'PRC':prc.item()}